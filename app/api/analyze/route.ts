...
**📐 좌표 측정 가이드 (매우 중요!):**
- 이미지를 10x10 그리드로 상상하세요 (가로 10칸, 세로 10칸 = 각 칸은 10%)
- 좌표는 이미지 전체를 기준으로 한 **퍼센트(%) 단위**로 계산하세요
- 측정 방법:
  1. **top**: 이미지 맨 위(0%)에서 요소의 상단까지의 거리
     예: 이미지의 1/4 지점 = 25%, 중간 = 50%, 3/4 = 75%
  2. **left**: 이미지 맨 왼쪽(0%)에서 요소의 왼쪽까지의 거리
     예: 왼쪽 끝 = 0%, 중앙 = 50%, 오른쪽 끝 = 100%
  3. **width**: 요소가 차지하는 가로 폭
     예: 이미지 전체 폭의 절반 = 50%, 1/4 = 25%
  4. **height**: 요소가 차지하는 세로 높이
     예: 버튼 하나 = 5-10%, 헤더 = 10-15%, 카드 = 20-30%

**측정 예시:**
- 상단 헤더: {top: 0, left: 0, width: 100, height: 10}
- 중앙 버튼: {top: 45, left: 35, width: 30, height: 8}
- 하단 텍스트: {top: 85, left: 10, width: 80, height: 5}

**⚠️ 정확도 체크리스트:**
- [ ] 요소가 실제로 그 위치에 있는가?
- [ ] 크기가 실제 요소의 크기와 비슷한가?
- [ ] 다른 요소와 겹치지 않는가?
- [ ] 이미지 밖으로 나가지 않는가? (0-100% 범위 내)
...
**📍 좌표 측정 시 주의사항:**
1. 이미지를 천천히 관찰하면서 문제 요소의 정확한 위치를 파악하세요
2. 10x10 그리드를 상상하여 퍼센트를 계산하세요
3. 요소의 경계를 정확하게 파악하여 width와 height를 측정하세요
4. 좌표가 이미지 범위(0-100%)를 벗어나지 않도록 확인하세요
5. 작은 요소(버튼, 텍스트)는 정확한 위치와 적절한 크기로 표시하세요

각 피드백 항목은 다음 정보를 포함해야 합니다:
- Issue ID (id)
- 위치 좌표 (coordinates) - **정확한 측정 필수!**
- 문제 유형 (type)
